    ...
    G: AsFd + Clone,
{
        ...
        E: RenderElement<R>,
    {
        ...
        if has_underlay {
            trace!(
                "failed to assign element {:?} to primary plane {:?}, already has underlay",
                element.id(),
                self.planes.primary.handle
            );
            return Ok(Err(None));
            ...
            .map(|state| {
                state
                    ...
                    .map(|other| {
                        ...
                        // here as we exclude the destition location from the test
        ...
        E: RenderElement<R>,
    {
        ...
        >| {
            ...
            if overlaps_with_plane_underneath {
                trace!(
                    "skipping direct scan-out on plane {:?} with zpos {:?}, element {:?} geometry {:?} overlaps with plane underneath", plane.handle, plane.zpos, element_id, element_config.geometry,
                );
                return Ok(Err(None));
            }
        ...
        for plane in self.planes.overlay.iter().filter(is_plane_compatible) {
            if let Ok(plane_assignment) = test_overlay_plane(plane, &element_config)? {
                trace!(
                    "assigned element {:?} geometry {:?} to compatible plane {:?} with zpos {:?}",
                    element_id,
        ...
        for (index, plane) in self.planes.overlay.iter().enumerate() {
            ...
            if element_config.failed_planes.overlay_bitmask & (1 << index) != 0 {
                trace!(
                    "skipping direct scan-out on plane {:?} with zpos {:?}, element {:?} geometry {:?}, test already known to fail", plane.handle, plane.zpos, element_id, element_config.geometry,
                );
        ...
        E: RenderElement<R>,
    {
        ...
        trace!("testing direct scan-out for element {:?} on plane {:?} with zpos {:?}: fb: {:?}, element_geometry: {:?}", element_id, plane.handle, plane.zpos, &element_config.buffer.fb, element_config.geometry);
