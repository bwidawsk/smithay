...
use crate::{
    backend::{
        ...
        renderer::{
            ...
            pixman::fourcc_to_pixman_format,
            sync::SyncPoint,
            utils::{CommitCounter, DamageBag, DamageSnapshot},
            Bind, Blit, DebugFlags, ExportMem, Frame as RendererFrame, Renderer, Texture,
        },
        SwapBuffersError,
    },
    ...
    G: AsFd + Clone,
{
        ...
        E: RenderElement<R>,
    {
        ...
        let (pdata, size, (format, width, height, stride)) = match element.underlying_storage(renderer) {
            ...
            None => {
                todo!("{:?} element has no underlying storage", element.kind());
            }
        };
        ...
        match cursor_buffer.map_mut::<_, _, Result<(), pixman::CreateFailed>>(
            ...
            0,
            0,
            cursor_buffer.width().expect("FIXME"),
            cursor_buffer.height().expect("FIXME"),
            |mbo| {
                /*
                let mut cursor_dst = pixman::Image::from_bits(
                    plane_format,
                    mbo.width() as usize,
                    mbo.height() as usize,
                ...
                */
                let mut cursor_dst =
                    pixman::Image::new(plane_format, mbo.width() as usize, mbo.height() as usize, false)?;
